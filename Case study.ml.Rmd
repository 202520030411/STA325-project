...
hist(log(data1$R_moment_4))#log transformation of Rmoment4
data$r4<-log(data1$R_moment_4)
...
hist(data$lgfr)
#scatterplots
plot(data$St,data$R_moment4, xlab="St", ylab="R_moment4")
pairs(r4~ St+Re+cfr,data=data)#scatterplot of paired predictors and response variable 
plot(data1$St,data1$R_moment_4)
...
lm.1<-lm(r4~St+Re+cfr,data=data)
summary(lm.1)
```
```{r}
#Ridge regression 
...
lm.2<-lm(r4~St+Re+Re*cfr,data=data)
summary(lm.2)
```
...
#perform regression 
dummy1 <- lm(data$r4 ~ St+Re+Sub, data = data)
summary(gamstep)
...
```{r}
#natural spline
gam1=lm(r4~ns(St)+ns(Re) +ns(cfr),data=data)
summary(gam1)

gam2<-lm(r4~ns(St)+ns(Re)+Sub, data=data)
summary(gam2)
```
...
lm.1=glm(r4~log(St)+Re+cfr,data=data)
  ridge.m4=cv.glmnet(x=train.x,y=train.y,alpha =0)
  lm.2=glm(r4~St+Re+Re*cfr,data=data)
  dummy1 =glm(r4 ~ St+Re+Sub, data = data)
  gamcfr<-glm(r4~ns(St)+ns(Re) +ns(cfr),data=data)
  gamdum<-glm(r4~ns(St)+ns(Re)+Sub, data=data)
results <- list()  # Store LOOCV results
mname<-list()#store model names
results[1]<-cv.glm(data=data, lm.1)$delta[1]
mname[1]<-"linear ols with st, re, cfr"
results[2]<- min(ridge.m4$cvm)
mname[2]<-"ridge regression with st, re cfr"
results[3]<-cv.glm(data=data, lm.2)$delta[1]
mname[3]<-"ols with st, re cfr, and interaction re*cfr"
results[4]<-cv.glm(data=data, dummy1)$delta[1]
mname[4]<-"ols regression with st, re, cfr and dummy variable sub"
results[5]<-cv.glm(data, gamcfr)$delta[1]
mname[5]<-"Generalized additive model with natural splines of st, re and cfr"
results[6]<-cv.glm(data, gamdum)$delta[1]
mname[6]<-"Generalized additive model with natural splines(ns) of st, ns of re and dummy variable sub "
results
...
#add interaction terms 
lm.4<-lm(r2~St+Re+Re*cfr,data=data)
summary(lm.4)

```
```{r}
#dummy variable 
dum2 <- lm(data$r2 ~ St+Re+Sub, data = data)
summary(dum2)
...
```{r}
#natural spline
gam1=lm(r2~ns(St)+ns(Re) +ns(cfr),data=data)
summary(gam1)
gam2<-lm(r2~ns(St)+ns(Re)+Sub, data=data)
summary(gam2)
```
...
ridge.m2=cv.glmnet(x=train.x,y=train.y2,alpha =0)
lm.4<-glm(r2~St+Re+cfr+Re*cfr,data=data)
dum2 <- glm(r2 ~ St+Re+Sub, data = data)
gamcfr<-glm(r2~ns(St)+ns(Re) +ns(cfr),data=data)
gamdum<-glm(r2~ns(St)+ns(Re)+Sub, data=data)  
results <- list()  # Store LOOCV results
